...
//            this.lock();
    ...
    public boolean lock(String ROOT_LOCK) {
        if (this.tryLock(ROOT_LOCK)) {
            System.out.println(Thread.currentThread().getName() + ":" + CURRENT_NODE + "获得锁成功");
            return true;
        } else {
            waitNodeCommit(WAIT_NODE, ROOT_LOCK);
            return false;
        }
    }
     ...
     * @param node 要被监听的节点
     */
    private void waitNodeCommit(String node, String ROOT_LOCK) {
        try {
     ...
     * @return 是否可以获得锁
     */
    public boolean tryLock(String ROOT_LOCK) {
        try {
            /*
                这里进行了一次改造：
